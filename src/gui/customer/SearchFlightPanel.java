...
public class SearchFlightPanel extends JPanel {
	...
	private Vector<String> headings = controller.AllFlights.getInstance().getColumnName();
	...
	public SearchFlightPanel(JFrame frame) {
		...
		columns.add(headings.get(model.Flight.NAME));
		columns.add(headings.get(model.Flight.DEPARTURE));
		columns.add(headings.get(model.Flight.ARRIVE));		
		...
		if (data != null) {			
			for (Iterator it = data.iterator(); it.hasNext();) {
				...
				if (!itemRow.get(model.Flight.SOURCE).equals(getSource())) {
					...
					row.add(itemRow.get(model.Flight.DEPARTURE));
