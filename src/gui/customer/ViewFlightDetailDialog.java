...
public class ViewFlightDetailDialog extends JDialog {
	...
	private void jbInit() {
		model.Flight f = controller.AllFlights.getInstance().getFlightDetail((String) flight.get(model.Flight.ID));
		...
		for (Iterator it = flightDetails.iterator(); it.hasNext();) {
			...
			if (v.get(model.FlightDetail.CLASSTYPE).equals(controller.ASRS.FIRST_CLASS)) {
				first = v;
			}

			if (v.get(model.FlightDetail.CLASSTYPE).equals(controller.ASRS.BUSSINESS_CLASS)) {
				bussiness = v;
			}

			if (v.get(model.FlightDetail.CLASSTYPE).equals(controller.ASRS.ECONOMIC_CLASS)) {
				economic = v;
			}
		}
		...
		jlabel8 = new JLabel("	Price: " + first.get(model.FlightDetail.PRICE));
		jlabel9 = new JLabel("	Seat Left: " + first.get(model.FlightDetail.SEATLEFT));
		jlabel10 = new JLabel("	Total Seats: " + first.get(model.FlightDetail.TOTALSEAT));

		jlabel11 = new JLabel("	Bussiness Class");
		jlabel12 = new JLabel("	Price: " + bussiness.get(model.FlightDetail.PRICE));
		jlabel13 = new JLabel("	Seat Left: " + bussiness.get(model.FlightDetail.SEATLEFT));
		jlabel14 = new JLabel("	Total Seats: " + bussiness.get(model.FlightDetail.TOTALSEAT));

		jlabel15 = new JLabel("	Economic Class");
		jlabel16 = new JLabel("	Price: " + economic.get(model.FlightDetail.PRICE));
		jlabel17 = new JLabel("	Seat Left: " + economic.get(model.FlightDetail.SEATLEFT));
		jlabel18 = new JLabel("	Total Seats: " + economic.get(model.FlightDetail.TOTALSEAT));
