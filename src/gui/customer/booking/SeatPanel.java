...
public class SeatPanel extends JPanel {
	...
	public SeatPanel(JFrame frame, AirplanePanel airplanePanel, int numOfRows, int numOfColumns, Vector flight) {
		...
		Object id = flight.get(model.Flight.ID);
		Vector v = controller.AllBookings.getInstance().get(model.Ticket.FLIGHTDETAIL, id);
		if (v != null) {
			for (Iterator it = v.iterator(); it.hasNext();) {
				Vector b = (Vector) it.next();
				Object book = b.get(model.Booking.BOOKINGNUMBER);
				Vector bts = controller.GetBookingTicket.getInstance().get(model.BookingTicket.BOOKINGNUMBER, book);
				Vector bt = (Vector) bts.get(0);
				Object key = bt.get(model.BookingTicket.TICKETNUMBER);
				Vector ts = controller.AllTickets.getInstance().get(model.Ticket.TICKETNUMBER, key);
				Vector t = (Vector) ts.get(0);
				if (b.get(model.Booking.STATUS).equals(ASRS.RESERVATION + "")) {
					reservation.add((String) t.get(model.Ticket.SEATNUMBER));
				} else if (b.get(model.Booking.STATUS).equals(ASRS.HOLDING + "")) {
					holdings.add((String) t.get(model.Ticket.SEATNUMBER));
				} else if (b.get(model.Booking.STATUS).equals(ASRS.CONFIRMATION + "")) {
					confirm.add((String) t.get(model.Ticket.SEATNUMBER));
				}
			}
		}
