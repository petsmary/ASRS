...
public class AirplanePanel extends JPanel {
	...
	private void goReservation(String firstName, String lastName, String email, String phone, Vector flight) {
		...
		String bookingNumber = year + "" + month + "" + day + controller.AllBookings.getInstance().getTable().size();
		String ticketNumber = ((String) flight.get(model.Flight.NAME)).substring(2, 5) + flight.get(model.Flight.ID)
		...
		v.add(email);
		v.add(phone);
		v.add(flight.get(model.Flight.ID));
		v.add(ASRS.RESERVATION);
		if (controller.AllBookings.getInstance().put(v)) {
			...
			if (controller.AllTickets.getInstance().put(v2)) {
				Vector v3 = new Vector();
				v3.add("");
				v3.add(ticketNumber);
				v3.add(bookingNumber);
				v3.add("false");
				if (controller.GetBookingTicket.getInstance().put(v3)) {
					Vector vector = controller.AllFlights.getInstance().getFlightDetail(model.FlightDetail.FLIGHTLID,
							flight.get(model.Flight.ID));
					for (Iterator it = vector.iterator(); it.hasNext();) {
						Vector v4 = (Vector) it.next();
						if (getClassType2().equals(v4.get(model.FlightDetail.CLASSTYPE))) {
							int seatl = Integer.parseInt((String) v4.get(model.FlightDetail.SEATLEFT)) - 1;
							v4.set(model.FlightDetail.SEATLEFT, seatl + "");
							controller.AllFlights.getInstance().changeFlightDetail(v4);
						}
	...
	public AirplanePanel(BookingFrame frame, Vector flight, String type) {
		...
		if (type.equals(ASRS.STRING_RESERVATION)) {
			...
			String bussinessRows = (String) vector.get(model.Airplane.BUSSINESS);
			String economicRows = (String) vector.get(model.Airplane.ECONOMIC);
			String columnRows = (String) vector.get(model.Airplane.COLUMN);
			String rows = (String) vector.get(model.Airplane.ROW);
