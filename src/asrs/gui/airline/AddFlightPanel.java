...
public class AddFlightPanel extends JPanel {
	...
	private void add() {
		if (flightName.getText().equals("") || airplane.getSelectedItem().equals("")
			...
			return;
		} else if (asrs.controller.AllFlights.getInstance().getFlight(asrs.model.Flight.NAME, flightName.getText()).size() != 0) {
			...
			if (asrs.controller.AllFlights.getInstance().putFlight(v)) {
				Vector vector = asrs.controller.AllFlights.getInstance().getFlight(asrs.model.Flight.NAME, flightName.getText());
				...
				v1.add(flight.get(asrs.model.Flight.ID));
				v2.add("");
				v2.add("2");
				...
				v3.add(flight.get(asrs.model.Flight.ID));
				asrs.controller.AllFlights.getInstance().putFlightDetail(v1);
				asrs.controller.AllFlights.getInstance().putFlightDetail(v2);
				asrs.controller.AllFlights.getInstance().putFlightDetail(v3);
	...
	private void changeSeats() {
		for (Iterator it = airplanesList.iterator(); it.hasNext();) {
			...
			if (v.get(asrs.model.Airplane.NAME).equals(airplane.getSelectedItem())) {
				int totalColumn = Integer.parseInt(((String) v.get(asrs.model.Airplane.COLUMN)));
				int first = Integer.parseInt(((String) v.get(asrs.model.Airplane.FIRST))) * totalColumn;
				int bussiness = Integer.parseInt(((String) v.get(asrs.model.Airplane.BUSSINESS))) * totalColumn;
				int economic = Integer.parseInt(((String) v.get(asrs.model.Airplane.ECONOMIC))) * totalColumn;
	...
	public AddFlightPanel(JFrame frame, Object key) {
		...
		airplanesList = asrs.controller.AllAirplanes.getInstance().getAirplane(asrs.model.Airplane.AIRLINE, key);
		Vector<String> airplanes = new Vector();
		idOfAirplane = new Vector();
		airplanes.add("");
		for (Iterator it = airplanesList.iterator(); it.hasNext();) {
			Vector v = (Vector) it.next();
			airplanes.add((String) v.get(asrs.model.Airplane.NAME));
			idOfAirplane.add(v.get(asrs.model.Airplane.ID));
			idOfAirplane.add(v.get(asrs.model.Airplane.NAME));
		}
